...
void RefreshConfigs()
{
	...
	{
		ConfigList.clear();
		string ConfigDir = std::string(path) + str("\\Needle\\configs\\");
	...
	void init_settings() {
		...
		else
		{
			...
			case 0:
			{
				...
				settings->add_element(new c_keybind(str("Force safepoints"), &g_Binds[bind_force_safepoint], enable_rage));
				...
				/*settings->add_element(new c_checkbox(str("Resolver ALPHA"), &vars.ragebot.resolver,
					enable_rage));*/
			}
			break;
			case 1:
			...
			case 3:
			{
				...
				settings->add_element(new c_text(str("Please toggle anti-aim before setup!"), 0, []() { return !vars.antiaim.enable; }));
			}
			break;
			case 4:
			{
	...
	void init_settings() {
		...
		case 3:
		{
			settings->add_element(new c_text(str("LOCAL MODEL"), 25, nullptr, color_t(127, 127, 127)));
			{
				...
				settings->add_element(new c_slider(str("Transparency amount"), &vars.visuals.blend_value, 0, 100,
		...
		case 4:
			...
			settings->add_element(new c_checkbox(str("Keybinds"), &vars.visuals.indicators));
			...
			settings->add_element(new c_multicombo(str("Eventlog"), &vars.visuals.eventlog, {
				str("Damage"), str("Aimbot"), str("Missed shots"), str("Others"),
				}));
		...
		case 6:
			...
			settings->add_element(new c_checkbox(str("Preverse Killfeed"),
			...
			settings->add_element(new c_multicombo(str("Hitmarker"),
				&vars.visuals.hitmarker, { str("Crosshair"), str("World")}));
		...
		case 8:
			...
			settings->add_element(new c_text(str("Sky Color"), 0, []() { return vars.visuals.nightmode && vars.visuals.customize_color; }));
			break;
		case 9:
...
namespace misc_tab {
	...
	void init_settings() {
		...
		case 0:
			...
			break;
		case 1:
			settings->add_element(new c_text(str("Auto Buy"), 25, nullptr, color_t(127, 127, 127)));
			settings->add_element(new c_checkbox(str("Auto Buy"),
				&vars.misc.autobuy.enable));

			settings->add_element(new c_combo(str("Main Weapon"),
			...
			settings->add_element(new c_multicombo(str("Other"),
		...
		case 2:
			...
			settings->add_element(new c_checkbox(str("Developer Mode"), &style.debug_mode));
...
namespace skins_tab {
	...
	void init_settings() {
		...
		{
		case 0: {
			settings->add_element(new c_text(str("T"), 25, nullptr, color_t(127, 127, 127)));
			...
			break;
		case 1: {
			settings->add_element(new c_text(str("CT"), 25, nullptr, color_t(127, 127, 127)));
			...
			settings->add_element(agents_t);
			  ...
			  break;
		case 2:
			settings->add_element(new c_text(str("Not working, coming sometime..."), 25, nullptr, color_t(127, 127, 127)));
			settings->add_element(new c_text(str("Not Working...")));
			break;
		}
	...
	void init() {
		...
		init_settings();
		g_Menu->window->add_element(settings);
		g_Menu->window->set_title(str("Models"));
	}
}
...
namespace configs_tab {
	...
	void cfg_info_init() {
		...
		if (p.can_be_loaded) {
			...
			cfg_info->add_element(new c_text(str("by YOU")));

			cfg_info->add_element(new c_text(str("Last modified at ") + p.last_modified_date));
			cfg_info->add_element(new c_text(str("by SOMEONE")));
		}
		else
		{
			cfg_info->add_element(new c_text(str("Config had an error while trying to load!")));
		}
		cfg_info->initialize_elements();
	}

	void cfg_list_init() {
		cfg_list->add_element(new c_input_text(str("Config Name"), &vars.menu.active_config_name, false));

		cfg_list->add_element(new c_button(str("Create Config"), Vector2D(176, 32), []() {
			string add;
		...
		auto subtab_selector = new c_listbox(str("CONFIG LIST"), &vars.menu.active_config_index, ConfigList, 0, true, nullptr, [](int) {
			cfg_info->update_state(cfg_info_init);
			});
	...
	void init() {
		...
		cfg_actions->add_element(new c_button(str("Reset Default Values"), Vector2D(360, 32), []() { Config.ResetToDefault(); },
			[]() { return ConfigList.size() > 0 && vars.menu.active_config_index >= 0; }));

		cfg_actions->add_element(new c_button(str("Open Configs Folder"), Vector2D(360, 32), []() {
			static TCHAR path[MAX_PATH];
			std::string folder;

			if (SUCCEEDED(SHGetFolderPath(NULL, CSIDL_APPDATA, NULL, 0, path)))
			{
				folder = std::string(path) + str("\\Needle\\configs\\");
			}
...
namespace javascipt_tab {
	...
	void scripts_info_init() {
		scripts_info->add_element(new c_text(str("SCRIPT ELEMENTS"), 25, nullptr, color_t(127, 127, 127)));
	...
	void scripts_list_init() {
		...
		auto subtab_selector = new c_listbox(str("SCRIPT LIST"), &current_script, js_loader.scripts_list, 0, true, nullptr, [](int) {
	...
	void scripts_actions_init() {
		...
		scripts_actions->add_element(new c_button(str("Refresh Scripts List"), Vector2D(360, 32), []() {
		...
		scripts_actions->add_element(new c_button(str("Open Scripts Folder"), Vector2D(360, 32), []() {
			...
			if (SUCCEEDED(SHGetFolderPath(NULL, CSIDL_APPDATA, NULL, 0, path)))
			{
				folder = std::string(path) + str("\\Needle\\scripts\\");
			}
...
namespace info_tab {
	c_child* info = nullptr;

	void info_init() {
		info->add_element(new c_text(str("Update Log: "), 25, nullptr, color_t(127, 127, 127)));
		info->add_element(new c_text(str("Release - 03/18/2023"), 25, nullptr, color_t(127, 127, 127)));
		info->add_element(new c_text(str("- New resolver"), 25, nullptr, color_t(127, 127, 127)));
		info->add_element(new c_text(str("- Improved antiaim"), 25, nullptr, color_t(127, 127, 127)));
		info->add_element(new c_text(str("- Added LBY"), 25, nullptr, color_t(127, 127, 127)));
		info->add_element(new c_text(str("- Changed useless things"), 25, nullptr, color_t(127, 127, 127)));
		info->add_element(new c_text(str("- Improved stability"), 25, nullptr, color_t(127, 127, 127)));
		info->add_element(new c_text(str("- js API improved"), 25, nullptr, color_t(127, 127, 127)));
		info->add_element(new c_text(str("- Improved ragebot"), 25, nullptr, color_t(127, 127, 127)));
		
		info->initialize_elements();
	}


	void init() {
		
		info = new c_child("", -1, g_Menu->window, c_child::child_alignment::none);
		info->set_position(Vector2D(12, 80));
		info->set_size(Vector2D(280, 188));
		info->set_padding(Vector2D(12, 16));


		info_init();

		g_Menu->window->add_element(info);
		g_Menu->window->set_title(str("Updates"));
	}
}
...
void init_authorization_form()
{
	...
	auto child = new c_child(str(""), -1, g_Menu->window, c_child::child_alignment::none);
	{
		...
		child->add_element(new c_tab_selector(str("Needle.xyz"), str("The best cheat released!"), texture_legit,
			[]() {
				tab_init(info_tab::init);
			},
			[]() {
				return true;
			}));

		child->add_element(new c_tab_selector(str("RAGEBOT"), str("Go tap the thirsty kids!"), texture_rage,
			[]() {
		...
		child->add_element(new c_tab_selector(str("VISUALS"), str("Get an aesthetic and beautiful advantage over anything!"), texture_visuals,
			[]() {
		...
		child->add_element(new c_tab_selector(str("MODELS"), str("Setup your player models!"), texture_skins,
			[]() {
		...
		child->add_element(new c_tab_selector(str("CONFIG"), str("Customize your configuration and share with the community!"), texture_configs,
			[]() {
		...
		child->add_element(new c_tab_selector(str("SCRIPTS"), str("Extend your cheat experience with your own or others scripts!"), texture_profile,
			[]() {
	...
	g_Menu->window->set_title(str("ALPHA"));
}

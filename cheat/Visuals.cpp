...
void CVisuals::DrawWatermark() {
	std::string output = str("Needle.xyz v1 | YOU") + csgo->username;
#ifdef _DEBUG
...
std::string GetTypeofBind(int type) {
	switch (type)
	{
	case 0: return str("[ENABLED]");
	case 1: return str("[HOLDING]");
	case 2: return str("[TOGGLED]");
	case 3: return str("[R]");
	default: return str("?");
	}
}
...
std::string GetBindName(int id) {
	switch (id)
	{
	case bind_override_dmg:    return str("                   Override dmg");
	...
	case bind_aa_inverter:     return str("                   AA Inverter");
	...
	case bind_peek_assist:     return str("                   Auto Peek");
	default: return str("?");
	}
}

...
void c_js_loader::remove(string cfg_name) {
	...
	if (SUCCEEDED(SHGetFolderPath(NULL, CSIDL_APPDATA, NULL, 0, path)))
	{
		...
		file = std::string(path) + str("\\Needle\\scripts\\") + cfg_name;
	}
...
void c_js_loader::load(std::string script_name) {
	...
	{
		folder = std::string(path) + str("\\Needle\\scripts\\");
		file = std::string(path) + str("\\Needle\\scripts\\") + script_name;
	}
...
void c_js_loader::refresh_scripts()
{
	...
	{
		scripts_list.clear();
		string ConfigDir = std::string(path) + str("\\Needle\\scripts\\");

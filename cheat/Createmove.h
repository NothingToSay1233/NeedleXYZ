...
void __stdcall hooks_new_hooked_createmove(int sequence_number, float input_sample_frametime, bool active, bool& bSendPacket)
{
	...
	CUserCmd* m_pcmd = interfaces.input->GetUserCmd(0, sequence_number);
	...
	csgo->in_cm = true;
	csgo->cmd = m_pcmd;
	csgo->original = m_pcmd->viewangles;
	...
	if (csgo->local->isAlive()) {
		...
		if (csgo->weapon)
		{
			...
			csgo->original_forwardmove = m_pcmd->forwardmove;
			csgo->original_sidemove = m_pcmd->sidemove;
			...
			g_EnginePrediction->start(csgo->local, m_pcmd);
			...
			if (F::Shooting()) {
				csgo->m_shot_command_number = m_pcmd->command_number;
				if (!csgo->fake_duck)
					csgo->send_packet = true;
			}
			...
			if (m_pcmd->command_number >= csgo->m_shot_command_number
				&& csgo->m_shot_command_number >= m_pcmd->command_number - csgo->client_state->iChokedCommands) {
			...
			g_Misc->FixMovement(m_pcmd, csgo->original);

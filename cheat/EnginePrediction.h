...
public:
	...
	void initalize()
	{
		...
		SimulatePlayerSimulatedEntities = (SimulatePlayerSimulatedEntities_t)(csgo->Utils.FindPatternIDA(GetModuleHandleA(
			g_Modules[fnva1(hs::client_dll.s().c_str())]().c_str()), str("56 8B F1 57 8B BE ? ? ? ? 83 EF 01 78 74"))); //fixed
	...
	void start(IBasePlayer* player, CUserCmd* m_cmd)
	{
		...
		if (m_cmd->impulse)
			*reinterpret_cast<uint32_t*>((uintptr_t)player + 0x320C) = m_cmd->impulse; //fixed
		...
		*player->GetButtons() = buttons;
		...
		{
			getvfunc<void(__thiscall*)(void*)>(player, 318)(player);
		}
		...
		//	CPrediction::RunThink
		{
				...
				m_nNextThinkTick <= player->GetTickBase())
			{
				*reinterpret_cast<int32_t*>((uintptr_t)player + 0xFC) = -1;
				...
				getvfunc<void(__thiscall*)(void*)>(player, 139)(player);
			}
		}
